...
const SimulationOverlay: React.FC<SimulationOverlayProps> = ({ active, nodes, variables, onToggleNode, onSetNode, onSetValue, onClose }) => {
    ...
    const getVarValue = (n: NodeData) => {
        if (!n.varName) return n.state;
        const v = variables.find(x => x.name === n.varName);
        return v ? v.value : n.state;
    };
    ...
    return (
        <div className="absolute inset-0 z-[100] flex flex-col bg-[#1a1a1a] bg-[radial-gradient(circle_at_center,_#222_0%,_#111_100%)]">
            {/* Top Panel - Outputs (Lights & Displays) */}
            <div className="p-5 flex justify-center items-center gap-10 shadow-[0_0_20px_rgba(0,0,0,0.5)] bg-panel-bg border-y border-border-col h-[140px] bg-[#2b2b2b] border-b-4 border-b-[#111] relative shrink-0">
                <div className="absolute top-1/2 left-0 right-0 h-2.5 bg-[#444] -mt-1.5 z-0 shadow-[inset_0_2px_5px_#000]"></div>
                
                {outputs.length === 0 && <div className="text-gray-500 text-xs italic z-10">No Active Outputs Assigned</div>}

                {outputs.map(n => {
                    const numeric = isNumeric(n);
                    const val = getVarValue(n);
            ...
            {/* Center Area */}
            <div className="flex-grow flex flex-col justify-center items-center text-[#444] text-xl uppercase tracking-[5px] font-bold shadow-[0_1px_1px_rgba(255,255,255,0.1)] pointer-events-none">
            ...
            <div className="p-5 flex justify-center items-center gap-10 shadow-[0_0_20px_rgba(0,0,0,0.5)] bg-panel-bg border-y border-border-col h-[200px] bg-gradient-to-b from-[#2d2d2d] to-[#1f1f1f] border-t-2 border-t-[#555] shrink-0">
                                ...
                                {numeric ? (
                                    ...
                                    <>
                                        {n.type === 'INPUT' && (
                                            ...
                                            >
                                                <div className={`w-full h-full rounded-sm flex flex-col justify-between items-center py-1 transition-colors ${val ? 'bg-gradient-to-b from-[#444] via-[#222] to-[#111]' : 'bg-gradient-to-b from-[#111] via-[#222] to-[#444]'}`}>
                                                    <span className="text-[10px] font-bold text-gray-500">I</span>
                                                    <div className={`w-2 h-2 rounded-full ${val ? 'bg-[#0f0] shadow-[0_0_8px_#0f0]' : 'bg-gray-800'}`}></div>
                                        ...
                                        {n.type === 'PUSH' && (
                                            <div 
                                                ...
                                                onTouchStart={(e) => { e.preventDefault(); onSetNode(n, true); }}
                                                onTouchEnd={(e) => { e.preventDefault(); onSetNode(n, false); }}
                                            ></div>
                                        )}
                                        {n.type === 'SENSOR' && (
                                            <div 
                                            ...
                                            >
                                                ...
                                                <div className={`absolute bottom-2 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full ${val ? 'bg-yellow-400 shadow-[0_0_10px_yellow]' : 'bg-red-900'}`}></div>
                                            </div>
                                        )}
                                    </>
                                )}
            ...
            <button 
                className="absolute top-4 right-4 bg-[#d9534f] text-white px-4 py-2 text-sm font-bold hover:bg-red-600 transition-colors shadow-xl rounded z-50"
                onClick={onClose}
            >
                EXIT SIMULATION
            </button>
