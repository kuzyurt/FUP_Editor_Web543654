...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, title, items, onClose }) => {
    return (
        <div 
            className="fixed z-50 min-w-[160px] bg-panel-bg border border-gray-600 shadow-2xl rounded-sm py-1"
            style={{ left: x, top: y }}
        >
            <div className="px-4 py-1.5 text-xs font-bold text-gray-500 border-b border-gray-700 mb-1">
                {title}
            </div>
            {items.map((item, idx) => {
