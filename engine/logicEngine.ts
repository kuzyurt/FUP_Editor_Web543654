...
import { resolveBlockType } from '../core/typeSystem';
...
export const evaluateCircuit = (
    ...
    // 0. Enforce "No Variable = No Work" rule for inputs
    // If an Input/Sensor has no variable, force state to false/0. 
    // PUSH buttons are excluded as they are momentary and manually driven.
    nodes.filter(n => ['INPUT', 'SENSOR'].includes(n.type)).forEach(n => {
        if (!n.varName) {
            n.state = false; // Reset state if unassigned
        }
    });
    ...
    nodes.forEach(n => {
        ...
        const boolArgs = rawArgs.map(v => !!v); 
